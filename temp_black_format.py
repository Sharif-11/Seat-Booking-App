...
from app.modules.shows.utils.get_seat_state import get_seat_state
...
class BookingService:
    ...
    async def create_booking(self, payload, user):
     ...
     try:
         ...
         seats = await get_seat_state(show_id)
    ...
    async def confirm_booking(self, booking_id, user_id, wallet_name, wallet_phone, idempotency_key=None):
        ...
        seat_state=await get_seat_state(booking_data["show_id"])
        await socket_manager.broadcast_to_room(
    room=booking_data["show_id"],
    event="seat_update",
    data=seat_state,
)
        return result
    ...
    async def failed_booking(self, booking_id, user_id):
        try:
            ...
            await BookingGateway().release_seat_locks(show_id, seat_ids)
            seat_state=await get_seat_state(show_id)
            await socket_manager.broadcast_to_room(
    room=show_id,
    event="seat_update",
    data=seat_state,
